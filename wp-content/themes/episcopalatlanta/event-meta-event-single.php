...
<div class="eventorganiser-event-meta">
	...
	<ul class="eo-event-meta">
		...
		<?php if ( get_the_terms( get_the_ID(), 'event-tag' ) && ! is_wp_error( get_the_terms( get_the_ID(), 'event-tag' ) ) ) { ?>
			<li><strong><?php esc_html_e( 'Tags', 'eventorganiser' ); ?>:</strong> <?php echo strip_tags(get_the_term_list( get_the_ID(), 'event-tag', '', ', ', '' )); ?></li>
		<?php } ?>
		...
		<?php $address = array_filter( eo_get_venue_address() ); ?>
			...
			<div class="event_addr">
			 ...
			 echo implode( ", ", $address ) . ' <a target="_blank" class="single_event_custom_map" href="http://maps.google.com?q='.implode( ", ", $address ).'">(map)</a>';
			     ?>
			 	</li>
			</div>
			<?php }?>
